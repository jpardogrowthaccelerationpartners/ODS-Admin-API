...
[TestFixture] public class ReadApplicationsByVendorTests {
    private static SqlServerUsersContext CreateContext() =>
        new(new DbContextOptionsBuilder<SqlServerUsersContext>()
            .UseInMemoryDatabase(databaseName: $"V3ReadAppsByVendor_{Guid.NewGuid()}").Options);
    ...
    [Test] public async Task GetVendorApplications_ReturnsOkWithList() {
        ...
        var result = await ReadApplicationsByVendor.GetVendorApplications(query, fakeGetDataStoreIds, vendor.VendorId);
