...
public class DeleteDataStoreContextCommandTests
{
    ...
    public void Execute_DeletesDataStoreContext()
    {
        ...
        ctx.SaveChanges();
        new DeleteDataStoreContextCommand(ctx).Execute(oic.OdsInstanceContextId);
    ...
    {
        using var ctx = CreateContext();
        Should.Throw<NotFoundException<int>>(() => new DeleteDataStoreContextCommand(ctx).Execute(9999));
    }
}
