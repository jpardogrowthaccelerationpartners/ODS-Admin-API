...
[TestFixture]
public class GetDataStoreIdsByApiClientIdQueryTests
{
    private static SqlServerUsersContext CreateContext() =>
        new(new DbContextOptionsBuilder<SqlServerUsersContext>()
            .UseInMemoryDatabase(databaseName: $"GetDataStoreIdsByApiClient_{Guid.NewGuid()}")
            .Options);

    [Test]
    public void Execute_WhenApiClientHasDataStores_ReturnsIds()
    {
        ...
        ctx.SaveChanges();
        var result = new GetDataStoreIdsByApiClientIdQuery(ctx).Execute(client.ApiClientId);
        result.Count.ShouldBe(1);
        result[0].ShouldBe(ods.OdsInstanceId);
    }

    [Test]
    public void Execute_WhenNoDataStores_ReturnsEmpty()
    {
        using var ctx = CreateContext();
        var result = new GetDataStoreIdsByApiClientIdQuery(ctx).Execute(9999);
