...
public class AddApiClientCommandTests
{
    private static SqlServerUsersContext CreateContext() =>
        new(new DbContextOptionsBuilder<SqlServerUsersContext>()
            .UseInMemoryDatabase(databaseName: $"AddApiClient_{Guid.NewGuid()}")
            .Options);
    ...
    [Test]
    public void Execute_WithValidModel_PersistsApiClient()
    {
        ...
        var result = new AddApiClientCommand(ctx).Execute(new AddApiClientModelStub
        {
            Name = "Client1", IsApproved = true, ApplicationId = app.ApplicationId
        }, DefaultOptions());
