...
public class GetResourcesByClaimSetIdQueryTests
{
    private static SqlServerSecurityContext CreateContext() =>
        new(new DbContextOptionsBuilder<SqlServerSecurityContext>()
            .UseInMemoryDatabase(databaseName: $"GetResourcesByClaimSet_{Guid.NewGuid()}")
            .Options);

    [Test]
    public void AllResources_WhenNoResourcesForClaimSet_ReturnsEmpty()
    {
        using var ctx = CreateContext();
        var cs = new ClaimSet { ClaimSetName = "Empty", IsEdfiPreset = false, ForApplicationUseOnly = false };
    ...
    [Test]
    public void SingleResource_WhenParentExists_ReturnsParentWithId()
    {
        using var ctx = CreateContext();
        var cs = new ClaimSet { ClaimSetName = "CS2", IsEdfiPreset = false, ForApplicationUseOnly = false };
        ctx.ClaimSets.Add(cs);
        var action = new Action { ActionName = "Read", ActionUri = "uri" };
        ctx.Actions.Add(action);
        var rc = new SecurityResourceClaim { ResourceName = "students", ClaimName = "students" };
        ctx.ResourceClaims.Add(rc);
        ctx.SaveChanges();
        ...
        var result = new GetResourcesByClaimSetIdQuery(ctx).SingleResource(cs.ClaimSetId, rc.ResourceClaimId);
    ...
    {
        var parent = new EdFi.Ods.AdminApi.Infrastructure.ClaimSetEditor.ResourceClaim { Id = 1, Name = "parent" };
        var child = new EdFi.Ods.AdminApi.Infrastructure.ClaimSetEditor.ResourceClaim { Id = 2, Name = "child", ParentId = 1 };

        var parents = new System.Collections.Generic.List<EdFi.Ods.AdminApi.Infrastructure.ClaimSetEditor.ResourceClaim> { parent };
        var children = new System.Collections.Generic.List<EdFi.Ods.AdminApi.Infrastructure.ClaimSetEditor.ResourceClaim> { child };
